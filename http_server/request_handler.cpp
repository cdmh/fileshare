...
request_handler::request_handler(const std::string& doc_root, http_query_callback http_query)
  : doc_root_(doc_root), http_query_(http_query)
{
}
...
void request_handler::handle_request(boost::shared_ptr<request> const &req, reply &rep, boost::shared_ptr<connection> const &conn)
{
  ...
  {
    if (!http_query_(req->headers, request_path, rep.headers, rep.content, conn))
    {
        rep.status = reply::ok;
        return;
    }
